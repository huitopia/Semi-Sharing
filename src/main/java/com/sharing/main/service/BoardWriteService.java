...
import com.sharing.main.dao.BoardDao;
...
public class BoardWriteService  implements CommandProcess {
	...
	public String requestProcess(HttpServletRequest request, 
		 ...
		 **/	
		String title = multi.getParameter("qTitle");
	    String writer = multi.getParameter("uNum"); 
	    String pass = multi.getParameter("pass");
		String content = multi.getParameter("qText");		
		
		/* 하나의 게시 글 정보를 저장하는 자바빈 객체를 생성하고 요청 데이터를
		 * Board 객체에 저장한다.
		 **/
		QNAPost board = new QNAPost();
		board.setqTitle(title);
		/* board.setuNum(writer); */
		/* board.setPass(pass); */
		board.setqText(content);	
		 ...
		 * System.out.println("업로드 된 파일명 : " + fileName); System.out.println("원본 파일명 : "
		 * + multi.getOriginalFileName("file"));
		 * 
		 * // 파일명이 존재하면 파일명을 지정하고 존재하지 않으면 null로 지정 한다. board.setFile(fileName != null ?
		 * fileName : null);
		 * 
		 * if(board.getFile() == null) { System.out.println("파일이 업로드 되지 않았음"); }
		 */
