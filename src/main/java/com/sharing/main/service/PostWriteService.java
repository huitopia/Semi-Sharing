...
public class PostWriteService implements CommandProcess {
	...
	private int uNum;
		...
		String ctCode = multi.getParameter("ctCode");
		String cCode = multi.getParameter("cCode");
		String lCode = multi.getParameter("lCode");
		String uNum = multi.getParameter("uNum");	
		
		/* 하나의 게시 글 정보를 저장하는 자바빈 객체를 생성하고 요청 데이터를
		 * Board 객체에 저장한다.
		 **/
		MPost mPost = new MPost();
		mPost.setpTitle(pTitle);
		mPost.setpText(pText);
		mPost.setCtCode(Integer.parseInt(ctCode));
		mPost.setcCode(Integer.parseInt(cCode));
		mPost.setlCode(Integer.parseInt(lCode));
		mPost.setuNum(Integer.parseInt(uNum));
			
		/* 사용자가 업로드한 파일 데이터 처리
		 * MultipartRequest 객체를 통해 파일 이름을 구하여 변수에 저장한다.
		 * 파일이 업로드 되지 않으면 fileName은 null 값을 받는다.
		 **/
		String fileName = multi.getFilesystemName("pPic");
		System.out.println("업로드 된 파일명 : " + fileName);
		System.out.println("원본 파일명 : " + multi.getOriginalFileName("pPic"));
		
		// 파일명이 존재하면 파일명을 지정하고 존재하지 않으면 null로 지정 한다.	 
		mPost.setpPic(fileName != null ? fileName : null);
		
		if(mPost.getpPic() == null) {		
			System.out.println("파일이 업로드 되지 않았음");		
		}	
		
		/* WriteDao 객체를 얻어 게시 글을 DB에 추가하기
